    ...
    <div className={`min-h-screen p-4 sm:p-6 ${isLightMode ? 'bg-gray-50' : 'bg-slate-900'}`}>
      <div className="max-w-2xl mx-auto">
        <div className="mb-6 sm:mb-8">
          <h1 className={`text-xl sm:text-2xl font-bold ${isLightMode ? 'text-gray-900' : 'text-white'}`}>Manage Account</h1>
          <p className={`${isLightMode ? 'text-gray-600' : 'text-gray-400'} mt-0.5 sm:mt-1 text-xs sm:text-sm`}>Update your login email and password.</p>
        </div>
          ...
          <form onSubmit={onUpdateEmail} className="p-6 space-y-4">
            ...
            <div className="pt-2">
              <button
                ...
                className="w-full sm:w-auto inline-flex items-center justify-center px-3 sm:px-4 py-2 text-sm sm:text-base bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 font-medium"
              >
        ...
        <div className={`rounded-xl shadow-sm overflow-hidden border ${isLightMode ? 'bg-white border-gray-100' : 'bg-slate-800 border-slate-700'}`}>
          <div className="px-6 py-5 border-b ${isLightMode ? 'border-gray-100' : 'border-slate-700'}">
            <h2 className={`${isLightMode ? 'text-gray-900' : 'text-white'} text-base sm:text-lg font-semibold`}>Change Password</h2>
            <p className={`${isLightMode ? 'text-gray-500' : 'text-gray-400'} text-xs sm:text-sm`}>Ensure your new password is strong.</p>
          </div>
          <form onSubmit={onUpdatePassword} className="p-4 sm:p-6 space-y-3 sm:space-y-4">
            {pwdError && (
              <div className="rounded-md bg-red-50 p-3 text-red-700 text-xs sm:text-sm">{pwdError}</div>
            )}
            {pwdMessage && (
              <div className="rounded-md bg-green-50 p-3 text-green-700 text-xs sm:text-sm">{pwdMessage}</div>
            )}
            <div>
              <label className={`${isLightMode ? 'text-gray-700' : 'text-gray-300'} block text-xs sm:text-sm font-medium mb-1`}>Current Password</label>
              <input
                type="password"
            ...
            <div>
              <label className={`${isLightMode ? 'text-gray-700' : 'text-gray-300'} block text-xs sm:text-sm font-medium mb-1`}>New Password</label>
              <input
                type="password"
                value={newPassword}
            ...
            <div>
              <label className={`${isLightMode ? 'text-gray-700' : 'text-gray-300'} block text-xs sm:text-sm font-medium mb-1`}>Confirm New Password</label>
              <input
                type="password"
